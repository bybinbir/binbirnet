<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white">Slider Yonetimi</h1>
      <button
        @click="openForm()"
    ...
    <div class="space-y-4">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="text-xs text-slate-400 mt-1">Siralama: {{ slide.order }}</div>
        </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 overflow-y-auto py-8" @click.self="showModal = false">
      <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 w-full max-w-lg shadow-2xl">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div>
            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Baslik (HTML destekli)</label>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Buton Ikon</label>
