<template>
  <div>
    <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white mb-8">Site Ayarlari</h1>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div v-if="activeTab === 'company'" class="space-y-4">
        <div>
          <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Sirket Adi</label>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Copyright Yili</label>
      ...
      <div v-if="activeTab === 'contact'" class="space-y-6">
        <div>
          <div class="flex items-center justify-between mb-3">
            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Telefon Numaralari</label>
          ...
          <div v-for="(phone, i) in form.contact.phones" :key="i" class="flex gap-2 mb-2 items-center">
            <input v-model="phone.label" placeholder="Label" class="w-32 border border-slate-200 dark:border-slate-700 rounded-xl p-2 text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-white" />
        ...
        <div>
          ...
          <div v-for="(email, i) in form.contact.emails" :key="i" class="flex gap-2 mb-2 items-center">
            <input v-model="email.label" placeholder="Label" class="w-32 border border-slate-200 dark:border-slate-700 rounded-xl p-2 text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-white" />
      ...
      <div v-if="activeTab === 'address'" class="space-y-4">
        ...
        <div>
          <div class="flex items-center justify-between mb-3">
            <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Adres Satirlari</label>
            <button @click="form.address.lines.push('')" class="text-sm text-primary font-semibold hover:underline">+ Satir Ekle</button>
          </div>
      ...
      <div v-if="activeTab === 'social'" class="space-y-4">
        ...
        <div v-for="(s, i) in form.social" :key="i" class="flex gap-2 mb-2 items-center">
          <input v-model="s.label" placeholder="Kisaltma (F, X, IG...)" class="w-20 border border-slate-200 dark:border-slate-700 rounded-xl p-2 text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-white" />
          <input v-model="s.aria" placeholder="Platform Adi" class="w-32 border border-slate-200 dark:border-slate-700 rounded-xl p-2 text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-white" />
      ...
      <div v-if="activeTab === 'links'" class="space-y-4">
        <div>
          <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Odeme Sayfasi URL</label>
        ...
        <div>
          <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Online Islem Merkezi URL</label>
...
const tabs = [
  { id: 'company', label: 'Sirket Bilgileri' },
  { id: 'contact', label: 'Iletisim' },
  { id: 'address', label: 'Adres' },
  { id: 'social', label: 'Sosyal Medya' },
  { id: 'links', label: 'Dis Linkler' },
]
