<template>
  <div>
    <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white mb-8">Navigasyon Yönetimi</h1>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6">
      ...
      <div v-if="activeTab === 'header'">
        <div class="flex items-center justify-between mb-4">
          <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Menü Öğeleri</label>
        ...
        <div class="space-y-3">
          <div v-for="(item, i) in form.header" :key="item.id" class="flex gap-2 items-center bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3">
            ...
            <input v-model="item.icon" placeholder="İkon" class="w-28 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-white" />
            <input v-model="item.label" placeholder="Etiket" class="w-32 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-white" />
            ...
            <label class="flex items-center gap-1 text-xs text-slate-500 whitespace-nowrap">
              <input type="checkbox" v-model="item.external" /> Dış
            </label>
      ...
      <div v-if="activeTab === 'footer'">
        <div class="flex items-center justify-between mb-4">
          <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Hızlı Erişim Linkleri</label>
        ...
        <div class="space-y-3">
          <div v-for="(item, i) in form.footerQuickLinks" :key="item.id" class="flex gap-2 items-center bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3">
            <input v-model.number="item.order" type="number" class="w-16 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm text-center bg-white dark:bg-slate-900 text-slate-900 dark:text-white" />
            <input v-model="item.label" placeholder="Etiket" class="w-40 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-white" />
